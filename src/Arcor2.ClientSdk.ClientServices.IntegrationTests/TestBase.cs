...
public class TestBase(ITestOutputHelper output) : IAsyncLifetime {
    ...
    public async Task SceneOpenObject(string type = "DobotM1") {
        ...
        await Session.Scenes.First().AddActionObjectWithDefaultParametersAsync(type, "TestObject", new Pose(new Position(), new Orientation()));
        await addAwaiter;
    ...
    /// <summary>
    /// Prepares the session with a started project with action point with an orientation, joints, and an action; and its scene.
    /// </summary>
    public async Task ProjectStartedObjectActionPointWithEntities(string type = "DobotM1") {
        ...
        var actionAddAwaiter = ap.Actions.CreateCollectionChangedAwaiter(NotifyCollectionChangedAction.Add).WaitForEventAsync();
    ...
    /// <summary>
    /// Prepares the session with a closed package using project with a simple program using one action point and the home action.
    /// </summary>
