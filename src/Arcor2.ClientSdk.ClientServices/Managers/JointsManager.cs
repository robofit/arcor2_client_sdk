...
namespace Arcor2.ClientSdk.ClientServices.Managers {
    ...
    public class JointsManager : LockableArcor2ObjectManager<ProjectRobotJoints> {
        ...
        public async Task UpdateAsync(List<Joint> joints) {
            ...
            var response =
                await Session.Client.UpdateActionPointJointsAsync(new UpdateActionPointJointsRequestArgs(Id, joints));
            if(!response.Result) {
        ...
        public async Task UpdateUsingRobotAsync() {
            await LibraryLockAsync(ActionPoint.Id);
            var response =
                await Session.Client.UpdateActionPointJointsUsingRobotAsync(
                    new UpdateActionPointJointsUsingRobotRequestArgs(Id));
            if(!response.Result) {
                await TryUnlockAsync(ActionPoint.Id);
                throw new Arcor2Exception($"Updating joints {Id} for action point {Id} using robot failed.",
                    response.Messages);
            }
        ...
        public async Task RemoveAsync() {
            await LibraryLockAsync(ActionPoint.Id);
            var response =
                await Session.Client.RemoveActionPointJointsAsync(new RemoveActionPointJointsRequestArgs(Id));
            if(!response.Result) {
                await TryUnlockAsync(ActionPoint.Id);
        ...
        public async Task RenameAsync(string newName) {
            await LibraryLockAsync(ActionPoint.Id);
            var response =
                await Session.Client.RenameActionPointJointsAsync(new RenameActionPointJointsRequestArgs(Id, newName));
            if(!response.Result) {
                await TryUnlockAsync(ActionPoint.Id);
