...
namespace Arcor2.ClientSdk.ClientServices.Managers {
    ...
    public class PackageManager : LockableArcor2ObjectManager<PackageMeta> {
        ...
        internal PackageManager(Arcor2Session session, PackageInfoData package, PackageStateData? state = null) : base(
            session, package.MapToPackageMeta(),
            package.PackageId) {
        ...
        /// </summary>
        /// <remarks>
        ///     The session must be in the menu. Invalid breakpoint IDs will not throw an exception, but rather lead to
        ///     <see cref="ExceptionOccured" /> event and closing of the package.
        /// </remarks>
        ...
        public async Task RunAsync(List<string> breakPoints, bool startPaused) {
            var response =
                await Session.Client.RunPackageAsync(new RunPackageRequestArgs(Id, startPaused, breakPoints));
            if(!response.Result) {
        ...
        /// <param name="startPaused">Should the package start paused? By default, "true".</param>
        /// <exception cref="Arcor2Exception"></exception>
        public async Task RunAsync(bool startPaused = true) => await RunAsync(new List<string>(), startPaused);
        ...
        ///     A list of breakpoints (action points). The package will pause before executing them. Invalid
        ///     IDs will lead to <see cref="ExceptionOccured" /> event.
        /// </param>
        /// >
        /// <exception cref="Arcor2Exception"></exception>
