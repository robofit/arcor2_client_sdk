...
namespace Arcor2.ClientSdk.ClientServices.Managers {
    ...
    public class ObjectTypeManager : LockableArcor2ObjectManager<ObjectType> {
        ...
        internal ObjectTypeManager(Arcor2Session session, ObjectTypeMeta meta, RobotMeta? robotMeta = null) : base(
            session, new ObjectType(meta, robotMeta), meta.Type) { }
        ...
        /// </value>
        public ObjectTypeManager? Parent =>
            parentCached ??= Session.ObjectTypes.FirstOrDefault(o => o.Id == Data.Meta.Base);
        ...
        /// </value>
        public ObjectTypeManager? SceneParent => sceneParentCached ??=
            Session.ObjectTypes.FirstOrDefault(o => o.Id == Data.Meta.NeedsParentType);
            ...
            if(!string.IsNullOrEmpty(Data.Meta.Base)) {
                ...
                if(Id != "Generic") {
                    Session.Logger?.LogWarn(
                        $"An object type '{objectType}' references non-existing parent '{Data.Meta.Base}'.");
                }
        ...
        public bool IsTypeOf(string objectType) {
            if(Data.Meta.Type == objectType &&
               objectType !=
               null! && // This is to prevent Generic being a type of empty string (lets assume empty string will never be a valid type)
        ...
        /// </summary>
        public bool IsGenericWithPose() =>
            IsTypeOf(Session.ObjectTypes.First(o => o.Data.Meta.Type == "GenericWithPose"));
        ...
        public bool IsCollisionObject() =>
            IsTypeOf(Session.ObjectTypes.First(o => o.Data.Meta.Type == "CollisionObject"));

        /// <summary>
        ///     Determines if object is derived from the builtin VirtualCollisionObject type.
        /// </summary>
        public bool IsVirtualCollisionObject() =>
            IsTypeOf(Session.ObjectTypes.First(o => o.Data.Meta.Type == "VirtualCollisionObject"));
        ...
        /// <summary>
        ///     Updates the object type according to the <paramref name="meta" /> and optional <paramref name="robotMeta" />
        ///     instances.
        /// </summary>
        ...
        internal void UpdateAccordingToNewObject(ObjectTypeMeta meta, RobotMeta? robotMeta = null) {
            if(Id != meta.Type) {
                throw new InvalidOperationException(
                    $"Can't update a ObjectTypeManager ({Id}) using a object type data object ({meta.Type}) with different type.");
            }
